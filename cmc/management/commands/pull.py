...
from cmc import models

def convert_price_to_num(text):
	return text.strip().replace('$', '').replace(',','').replace('<','').replace('--','0').replace('...','00000000')
		...
		for tr in trs:
			...
			if len(cols) > 3:
				...
				try:
					...
					cd.crypto = c
					cd.rank_num = count
					cd.price = convert_price_to_num(price)
					cd.market_cap = convert_price_to_num(market_cap)
					cd.volume = convert_price_to_num(volume)
					cd.save()
			...
			if not erro:
				...
				while True:
					try:
						...
						print(f'Stale Element erro ocorreu: {e}')
						count += 1
						pass
